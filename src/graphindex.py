...
# @author : cat
# @date   : 2023/2/8.
def locatetomsuornot(vrdlist,a,runlist, refchrlist):
            ...
            if inschr ==  achr  and aloc>insst and aloc<insed:
                ...
                if posheadchr in refchrlist and  postailchr  in refchrlist:
                    ...
                    if posheadchr not in refchrlist and postailchr  in refchrlist:
                        ...
                        a = locatetomsuornot(vrdlist,poshead,runlist, refchrlist)
                    if postailchr not in refchrlist and posheadchr in refchrlist:
                        vrdlist.append(poshead)
                        #print("tail",poshead,end = ",")
                        a = locatetomsuornot(vrdlist,postail,runlist, refchrlist)
...
def graphindex(goin):
    ...
    for i in genelistlnotonelevel:
        ...
        for j in i.split()[1:]:
            ...
            c +=1
            vrdlist = []
            locatetomsuornot(vrdlist,j,genecleanlist,reflist)
            if vrdlist == []:
                donout ="yes"
                break
            else:
